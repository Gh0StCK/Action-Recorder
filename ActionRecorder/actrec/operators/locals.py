...
def ar_local_category_items(self, context):
    """Список категорий для EnumProperty."""
    ActRec_pref = get_preferences(context)
    if ActRec_pref is None:
        return []
    ...
    for idx, cat in enumerate(categories):
        # cat.id и cat.label берутся из AR_category (properties/categories.py)
        name = cat.label or f"Category {idx + 1}"
        items.append((cat.id, name, ""))

    # EnumProperty всегда должен возвращать список кортежей (id, name, description)
    return items
...
class AR_OT_local_to_global(Operator):
    ...
    category_id: EnumProperty(
        name="Category",
        description="Target global category",
        items=ar_local_category_items,
    )
    ...
    def invoke(self, context: Context, event: Event) -> set[str]:
        ActRec_pref = get_preferences(context)
        cats = ActRec_pref.categories

        # Попробуем подставить выбранную по умолчанию
        if len(cats):
            # Пытаемся найти категорию, у которой selected = True
            selected_cat = None
            for cat in cats:
                # AR_category.selected использует свои get/set и тут безопасен
                if getattr(cat, "selected", False):
                    selected_cat = cat
                    break

            # Если ничего не выбрано — берём первую
            if selected_cat is None:
                selected_cat = cats[0]

            self.category_id = selected_cat.id
    ...
    def draw(self, context: Context) -> None:
        ...
        if len(categories):
            layout.prop(self, "category_id", text="Category")
        else:
            box = layout.box()
            col = box.column()
            col.scale_y = 0.9
    ...
    def execute(self, context: Context) -> set[str]:
        ...
        # Пытаемся найти категорию по выбранному id
        category = categories.get(self.category_id, None)
        ...
        self.local_to_global(
            ActRec_pref,
            category,
            ActRec_pref.local_actions[ActRec_pref.active_local_action_index]
        )

        if ActRec_pref.local_to_global_mode == 'move':
            functions.remove_local_action_from_text(
                ActRec_pref.local_actions[ActRec_pref.active_local_action_index]
            )
    ...
    def execute(self, context: Context) -> set[str]:
        ...
        while bpy.ops.ed.redo.poll() and redo_steps > 0 and len_reports > i:
            ...
            if bpy_type == CONTEXT_REPORT:
                ...
                # Второй redo, если состояние совпадает
                new_copy = functions.create_object_copy(context, parent, name)
                if bpy.ops.ed.redo.poll() and copy_dict == new_copy:
                    bpy.ops.ed.redo()
                    redo_steps -= 1
                    context = bpy.context

                # Теперь безопасно добавляем запись
                if copy_dict:
                    # Объект найден → делаем улучшенный report
                    data.append(functions.improve_context_report(context, copy_dict, parent, name, value))
                else:
                    # Объект не найден → fallback, чтобы не ломать макрос
                    data.append(f"bpy.context.{'.'.join(parent)}.{name} = {value}")
